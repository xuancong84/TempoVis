...
Visualization::Visualization(GLwindow *GLmain, BYTE *psData){
...
inline void	Visualization::SetCosineArray(int tempoInd){
...
void	Visualization::init(){
	...
	TempoEDBuffer	=	new LoopBuffer( TempoBufferLength*TempoPrecision*3, TempoBufferLength*TempoPrecision*2, sizeof(FLOAT), TempoPrecision );
...
int	Visualization::addData(TimedLevel *pLevels){
...
inline void Visualization::phase_slide(int phaseDiff){
	if( phaseDiff ){
...
inline void Visualization::phase_change(int phaseInd, int n_added){
	if (isIntegerMultiple(PhaseDiff(phaseInd, last_phase_index, last_tempo_index), last_tempo_index)){
...
void	Visualization::DrawAll(TimedLevel *pLevels, int n_added){
	...
	glPointSize(SPRITESIZE*pGLwin->Height/1024.0f);
		...
		if (est_spec.size() && g_mode & 2){
			for (int x = 0; x<est_spec.size(); x++){
				...
				if (x<est_fact.size() && x<est_fact2.size())
					pGLwin->glXYPrintf(pGLwin->Width >> 1, pGLwin->Height*x / est_spec.size(),
