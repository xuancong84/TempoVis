// TempoVisdll.cpp : Implementation of DLL Exports.
...
OBJECT_ENTRY(CLSID_TempoVis, CTempoVis)
END_OBJECT_MAP()
...
BOOL WINAPI DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID /*lpReserved*/)
{
    ...
    {
        _Module.Init(ObjectMap, hInstance, &LIBID_TEMPOVISLib);
...
{
	if (g_tempoVis){
		g_tempoVis->pVisual->pri_tempoIndex = 0;
			...
			if (g_tempoVis->record_state){
				...
				if (fp){
					...
					if (fsize > g_tempoVis->curr_media_duration*g_wavfmt.nAvgBytesPerSec*0.8f
						...
						HANDLE hThread;
						SetThreadPriority(
							hThread = CreateThread(NULL, 0, PostEstThreadFunc, new string(g_tempoVis->curr_media_name), 0, (DWORD*)&fsize),
							THREAD_PRIORITY_IDLE
						);
						g_tempoVis->thread_id2handles[fsize] = hThread;
					}
				}
			}
		...
		if (preset_tempo < 0 && g_mode & 2 && g_tempoVis->curr_media_duration < MAXWAVLENGTH){
			HANDLE hThread;
			SetThreadPriority(
				hThread = CreateThread(NULL, 0, PreEstThreadFunc, new string(str), 0, (DWORD*)&size),
				THREAD_PRIORITY_IDLE
			);
			g_tempoVis->thread_id2handles[size] = hThread;
		}
		else if (preset_tempo >= 0)	g_status = all_status[0];

		g_tempoVis->pVisual->reset(NULL,0);
		delete str;
	}
	else if (pMedia){
